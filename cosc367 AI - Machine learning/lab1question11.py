...
class SlidingPuzzleGraph(Graph):
    ...
    def is_goal(self, state):
        ...
        #goal state is the top left corner is blank then numbers are 1 through 8 in order
        n = len(state)
        # print(n)
        # print("state is", self.starting_state)
        flat_state_list = list(itertools.chain.from_iterable(state))

        state_check = [BLANK]

        for i in range(1, len(flat_state_list)):
            state_check.append(i)


        if flat_state_list == state_check:
            return True

        #combine the state in to a single list with "" at the front and the compare it with the ordered equilivant
        #
